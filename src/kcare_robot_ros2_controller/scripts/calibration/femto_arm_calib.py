...
class ImageSaverNode(Node):
    ...
    def save_images(self):
        ...
        arm_pose = self.rbutils.get_robot_pose()
        elev_pose = self.rbutils.get_elev_pose()
        ...
        if head_pose is not None:
            ...
            robot_state = {
                ...
                'arm_pose': {
                    'x': float(arm_pose['pose'][0]),
                    'y': float(arm_pose['pose'][1]),
                    'z': float(arm_pose['pose'][2]),
                    'rx': float(arm_pose['pose'][3]),
                    'ry': float(arm_pose['pose'][4]),
                    'rz': float(arm_pose['pose'][5])
                },
                'lift': float(elev_pose)  # float 하나
            }
            with open(robot_json_filename, 'w') as json_file:
                json.dump(robot_state, json_file)
