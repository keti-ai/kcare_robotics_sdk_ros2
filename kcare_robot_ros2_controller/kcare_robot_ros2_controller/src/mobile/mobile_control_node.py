...
class Mobile_Controller(Node):
    ...
    def is_goal_reached(self, target_x, target_y, target_yaw):
        if self.current_robot_pose is None:
            return False # 현재 로봇 pose가 없다면 false

        current_x = self.current_robot_pose.pose.position.x
        current_y = self.current_robot_pose.pose.position.y
        current_yaw = self.quaternion_to_yaw(
            self.current_robot_pose.pose.orientation.x,
            self.current_robot_pose.pose.orientation.y,
            self.current_robot_pose.pose.orientation.z,
        ...
        # 임계값 이내인지 확인
        linear_reached = distance_error <= self.goal_tolerance_xy
        angular_reached = abs(angular_error) <= self.goal_tolerance_yaw

        if linear_reached and angular_reached:
            return True
        else:
                ...
                if x is not None and y is not None and theta is not None:
                    ...
                    if request.wait:
                        ...
                        while True:
                            ...
                            # ✅ 로봇이 움직이는지 여부와 목표 도달 여부를 동시에 확인
                            # ✅ is_goal_reached 함수에 목표 위치(x, y, theta)를 직접 전달
                            if not self.get_moving() and self.is_goal_reached(x, y, theta):
                                self.get_logger().info(f"Moving Complete: Reached target pose ({x:.2f}, {y:.2f}, {math.degrees(theta):.2f} deg).")
                                break
