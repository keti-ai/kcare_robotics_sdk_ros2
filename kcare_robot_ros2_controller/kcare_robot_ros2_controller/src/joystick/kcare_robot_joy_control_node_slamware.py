...
class KcareRobotJoyControlNode(Node):
    ...
    def __init__(self):
        ...
        self.timer_joy = self.create_timer(0.02, self.timer_callback_joy)
        self.timer_head = self.create_timer(0.05, self.timer_callback_head)
        self.timer_lm = self.create_timer(0.1, self.timer_callback_lm)
    ...
    def timer_callback_joy(self):
        if self.last_joy_msg:
            if self.joy_cmd_on:
                if self.last_joy_msg.axes[1] or self.last_joy_msg.axes[2]:
    ...
    def timer_callback_head(self):
        if self.joy_cmd_on:
            if self.last_joy_msg.axes[4] or self.last_joy_msg.axes[5]:
                    # 헤드
                    head_pose_msg = HeadCommand()
                    # head_pose_msg.control_type = 'velocity'
                    head_pose_msg.control_type = 'position'
                    
                    head_pose_msg.rz = self.last_joy_msg.axes[4] * -5
                    head_pose_msg.ry = self.last_joy_msg.axes[5] * 5
